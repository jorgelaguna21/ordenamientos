...
public:
    ...
    void OrdenarPorPromedio()
    {
        ...
        for(int i = 0; i<n; i++){
            ...
            // Mientras el promedio de la solicitud actual sea menor al de la solicitud anterior
            while(j > 0 && aux.promedio > solicitudes[j - 1].promedio)
            {
                solicitudes[j] = solicitudes[j - 1];
                j--;
            }

            // Colocar la solicitud en su posición correcta
            if(i != j)
            {
                solicitudes[j] = aux;
            }
